...
- **多维度分析**能够从数据分布、稳健性、峰态特征、注意力、Decoder层级以及全模型输出差异等多个维度，精准评估层敏感度。详细情况如下：
  - **线性层**衡量算法：`std`、`quantile`、`kurtosis`
  - **attention结构**衡量算法：`attention_mse`
  - **Decoder层级**衡量算法：`mse_layer_wise`
  - **模型级输出**衡量算法：`mse_model_wise`
...
| `--metrics`           | `str`       | `"kurtosis"`    | 分析使用的度量算法，可选值：`"std"`, `"quantile"`, `"kurtosis"`, `"attention_mse"`, `"mse_layer_wise"`, `"mse_model_wise"`                  | `"kurtosis"`                             |
...
- **mse_layer_wise**: 层级输出 MSE 算法，基于量化前后前向**每层Decoder的线性层输出均值**差异，用于从层级输出角度衡量量化敏感度。
...
- **推荐场景**: 希望从模型最终输出角度评估各层量化影响的场景，无需模型适配；结果按 **Decoder 层（block）** 排序，适合做**整层回退
  **或按结构做**整块回退**（例如整个 attention / 整个 mlp）。

> [!note] 说明
> - `pattern` 参数决定了参与量化误差对比的具体结构范围。由于不同的 `pattern`配置会导致量化对象产生差异，因此最终的误差排序结果也会随之改变。
> - 不同模型的前向传播过程不同，只有当前 Decoder 层的输出可以直接作为下一层输入时能够使用；若层间输入输出无法对齐，会报错显示不支持。

#### mse_layer_wise (Layer-wise MSE) - 层级 MSE 算法

##### 算法原理

- 对同一批校准样本分别执行**量化前（浮点）**与**量化后**两次前向，采集每个DecodeLayer中线性层的输出。
- 计算每个线性层的输出的MSE，然后按照DecodeLayer层级进行拆分汇聚，采用线性层的MSE的均值进行汇聚计算得分，分数越大表示该层量化后对最终输出影响越大、越敏感。

##### 适用场景

- **推荐场景**: 希望从DecodeLayer层级输出MSE角度评估各层量化影响的场景，无需模型适配；结果按 **Decoder 层（block）** 排序，适合做
  **整层回退**或按结构做**整块回退**（例如整个 attention / 整个 mlp）。

> [!note] 说明
> - `pattern` 参数决定了参与量化误差对比的具体结构范围。由于不同的 `pattern`配置会导致量化对象产生差异，因此最终的误差排序结果也会随之改变。
> - MOE模型在量化模型和浮点模型中相同的校准集激活的专家可能不同，对于浮点模型和量化模型前向传播激活的专家不同的场景，当前算法无法计算其MSE，敏感层分析时会忽略对应线性层影响。
