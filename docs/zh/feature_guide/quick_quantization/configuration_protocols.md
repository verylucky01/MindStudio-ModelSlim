...
一键量化功能提供了两种使用方式供用户选择，并根据用户选择的方式来判断使用的量化配置协议。
...
不支持通过dataset字段配置校准数据集。由于多模态生成模型的量化校准数据处理方式与大语言模型存在明显差异，将通过识别指定dump_data_dir目录中是否存在名为"calib_data.pth"的校准数据，选择直接加载或自动获取并保存两种方式。详见[dump_config - 校准数据捕获配置](configuration_protocols.md#dump_config---校准数据捕获配置)
