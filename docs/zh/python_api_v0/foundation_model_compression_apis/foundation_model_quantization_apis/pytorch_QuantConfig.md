...
| w_method | 输入 | 选择权重量化策略。| 可选。<br>数据类型：str。默认为'MinMax'，可选值：'MinMax','GPTQ','HQQ','NF'。<br>MinMax、HQQ和NF支持Data-Free。<br>GPTQ不支持Data-Free。<br>NF不支持per-tensor,per-channel,per-group,lowbit,通信量化，kvcache量化，离群值抑制混合使用。|
