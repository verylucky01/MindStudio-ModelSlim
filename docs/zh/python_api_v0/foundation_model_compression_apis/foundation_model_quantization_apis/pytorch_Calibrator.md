...
| disable_level | 输入 | 自动回退等级，在模型精度损失大可以适当提升等级，但回退层数不可以大于模型总层数。| 可选。<br>数据类型：str。<br>配置示例如下：(1)'L0':默认值，不执行回退。(2)'L1'：回退1层。(3)'L2'：回退2层。(4)'L3'：回退3层。(5)'L4'：回退4层。(6)'L5'：回退5层。<br>以此类推。|
...
由 disable_names/disable_level（自动回退逻辑）共同决定。disable_names是QuantConfig的参数，可参考[QuantConfig参数说明](./pytorch_QuantConfig.md#参数说明)。
