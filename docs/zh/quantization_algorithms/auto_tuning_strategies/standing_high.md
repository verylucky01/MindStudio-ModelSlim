...
- **process**: 处理器列表，定义量化处理的处理器配置（linear_quant、iter_smooth等）
- **save**: 保存器列表，定义量化结果的保存方式（ascendv1_saver等）
- **dataset**: 校准数据集配置，指定校准数据集文件名

**配置要求**：

- 必须包含至少一个 `linear_quant` 处理器
- 可以包含其他处理器（如离群值抑制处理器）
...
**作用**: 定义策略的元数据信息，用于标识和分类量化配置。若已在启动自动调优前设置环境变量 `MSMODELSLIM_CUSTOM_PRACTICE_REPO`，且调优在精度达标时将最终量化配置保存到该路径对应的实践库中，则后续可以通过使用一键量化命令，只需指定与调优一致的 `quant_type`（保持在同一 `MSMODELSLIM_CUSTOM_PRACTICE_REPO` 环境），工具会从该实践库中自动检索并匹配适用的量化配置。
...
1. **模板配置要求**：`template` 中必须包含至少一个 `linear_quant` 处理器，否则会抛出配置错误。

2. **回退级别**：回退级别由量化回退层选择器根据模型结构和校准数据自动计算，级别越高，回退到浮点的层越多。回退级别为0表示所有层都量化，不回退任何层。

3. **推理引擎支持**：需要注意推理引擎对任意回退的支持情况。一般情况下，单算子模式下 vLLM-Ascend 支持任意回退，但使用混合算子时可能不支持任意回退。在使用 Standing High 算法时，请确保配置的推理引擎（如 vLLM-Ascend）能够支持量化回退功能。
