...
作为 Processor 使用，需配置 `type: "adapt_rotation"` 和 `stage: 1` 或 `stage: 2`，不同阶段的配置参数有所差异。Stage1 置于 `spec.prior` 的 process 列表中并配置该阶段的 dataset；Stage2 置于 `spec.process` 主流程列表中。YAML配置示例如下：
