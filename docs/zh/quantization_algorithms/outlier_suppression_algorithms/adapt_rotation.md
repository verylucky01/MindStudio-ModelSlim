...
- **核心思想**：在固定 Hadamard 矩阵的基础上，通过 Newton-Schulz 迭代求解正交极因子，学习一个额外的正交旋转 R，使得在给定激活数据上经量化-反量化后的重构误差最小。
- **与 QuaRot 的关系**：使用 Adapt Rotation 的前提是模型已适配 QuaRot 接口。Adapt Rotation 与 QuaRot 共享相同的旋转框架与模型适配接口（`QuaRotInterface`）。Stage1 负责基于校准数据优化旋转矩阵；Stage2 将优化后的矩阵应用到 QuaRot 中，替代默认的 Hadamard 矩阵。
...
算法在 `msmodelslim/processor/adapt_rotation/` 中实现，核心类包括：
...
- `HadamardOptimizer`：迭代 Hadamard 优化器，位于 `iterative_hadamard_optimization.py`。
...
- **模型架构要求**：模型必须支持 `QuaRotInterface` 接口，与 QuaRot 相同。
...
| block_size | 块大小 | `int` | `-1` 表示 hidden_dim，或大于 0 的 2 的幂 | `-1` |
...
| online | 是否启用在线旋转 | `bool` | 与 QuaRot 相同 | `False` |
| block_size | 块大小 | `int` | 与 QuaRot 相同 | `-1` |
| down_proj_online_layers | 使用在线旋转的 down_proj 层索引 | `array[int]` | 与 QuaRot 相同 | `[]` |
| max_tp_size | 最大张量并行大小 | `int` | 与 QuaRot 相同 | `4` |

## 模型适配

Adapt Rotation 复用 QuaRot 的模型适配接口：

- **QuaRotInterface**：必须实现，Stage1 与 Stage2 均依赖 `get_ln_fuse_map()`、`get_rotate_map(block_size)` 等。
- **LAOSOnlineRotationInterface**：仅在 Stage2 配置 `online: True` 时需要实现。

适配步骤与 QuaRot 相同，可参考 [QuaRot 模型适配](quarot.md#模型适配)。
...
**现象**：`adapter does not support QuaRotInterface`。

**解决方案**：确保模型适配器实现 `QuaRotInterface` 接口，与 QuaRot 相同。
