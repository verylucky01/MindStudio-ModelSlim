...
| 文件                                                                                                  | 核心类/函数                                        | 职责                                                 |
| ----------------------------------------------------------------------------------------------------- | -------------------------------------------------- | ---------------------------------------------------- |
| [`processor.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/processor.py)                     | `AWQProcessor`, `AWQProcessorConfig`               | Processor入口，管理预处理/后处理流程，自动发现LCA    |
| [`api.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/api.py)                                 | `awq()`, `awq_impl_*()`                            | 各子图类型的AWQ分发与实现                            |
| [`best_scales_search.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/best_scales_search.py)   | `AWQSearcher`, `AWQBestScalesSearcher`             | 缩放因子网格搜索逻辑                                 |
| [`awq_stats_collector.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/awq_stats_collector.py) | `AWQStatsCollector`                                | 激活统计信息收集（均值、kwargs缓存），使用全局上下文 |
| [`common.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/common.py)                           | `AWQConfig`, `AWQContext`, `offload()`, `onload()` | 算法配置、运行时上下文、张量设备迁移工具             |
| [`interface.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/processor/anti_outlier/awq/interface.py)                     | `AWQInterface`                                     | 模型适配器需要实现的抽象接口                         |
...
**参考实现：** 可参考 [`msmodelslim/model/qwen2/model_adapter.py`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/model/qwen2/model_adapter.py) 中的 [`Qwen2ModelAdapter`](https://gitcode.com/Ascend/msmodelslim/blob/master/msmodelslim/model/qwen2/model_adapter.py#L42) 实现。
