...
- **核心思想**：核心思想是“协同优化”。通过 [QuaRot](../outlier_suppression_algorithms/quarot.md) 和 [Iterative Smooth](../outlier_suppression_algorithms/iterative_smooth.md) 技术对激活分布进行平滑处理，有效抑制离群值，为后续[线性量化](linear_quant.md)创造良好条件；再利用 [AutoRound](autoround.md) 自适应功能为不同权重确定最优舍入策略，从而提高大模型在低比特量化场景的精度。
